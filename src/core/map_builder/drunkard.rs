...
use crate::core::map::{map_idx, Map};
use crate::core::map_builder::{
    MapArchitect, MapBuilder, Point, TileType, SCREEN_HEIGHT, SCREEN_WIDTH,
};
use crate::core::prelude::{Algorithm2D, DijkstraMap, DistanceAlg, RandomNumberGenerator};
...
impl MapArchitect for DrunkardsWalkArchitect {
    fn new(&mut self, rng: &mut RandomNumberGenerator) -> MapBuilder {
        ...
        while floor_size < DESIRED_FLOOR {
            let drunkard_point =
                &Point::new(rng.range(0, SCREEN_WIDTH), rng.range(0, SCREEN_HEIGHT));
            self.drunkard(drunkard_point, rng, &mut mb.map);
